...
<head>
    ...
    <style>
        ...
        .logo-box { flex: 1; display: flex; align-items: center; justify-content: flex-start; gap: 10px; font-family: sans-serif; }
        .logo-box img { width: 35px; height: 35px; }
        .logo-box h2 { margin: 0; color: var(--primary, #4a90e2); font-weight: 700; font-size: 1.2rem; }
        
        .nav-links { flex: 2; display: flex; justify-content: center; gap: 5px; }
...
<?php else: ?>
    ...
    <nav>
        ...
        <div class="nav-controls">
            ...
            <button class="hamburger btn-outline btn-sm" id="hamburgerBtn" style="display:none;"><svg class="icon"><use href="#icon-menu"></use></svg></button>
        </div>
    </nav>
    ...
    <div class="modal" id="addItemModal">
        <div class="modal-content">
            ...
            <form id="addStockForm">
                <input type="text" id="addName" name="name" placeholder="Medication Name" required>
...
function changeTextSize(delta) {
    baseTextSize += (delta * 2); // Increase/decrease by 2px
...
window.addEventListener('resize', () => {
    if(document.getElementById('hamburgerBtn')) {
        document.getElementById('hamburgerBtn').style.display = window.innerWidth <= 850 ? 'flex' : 'none';
    }
});
...
document.querySelectorAll('[data-page]').forEach(btn => btn.addEventListener('click', () => {
    ...
    document.getElementById('navLinks').classList.remove('show'); // Close mobile menu on click
        ...
        <div class="card">
            ...
            ${attn.length === 0 ? '<p>No alerts.</p>' : attn.map(i => `
                <div class="list-item" style="border-left: 5px solid ${i.color}">
                    <strong>${e(i.name)}</strong> - ${e(i.reason)}
                </div>`).join('')}
        </div>`;
}
...
async function renderInventory(c) {
    ...
    c.innerHTML = `
        ...
        <div id="invList">${inv.map(i => `
            <div class="list-item">
                <div>
                    <strong>${e(i.name)} ${e(i.strength)}</strong> ${i.status === 'in_use' ? '<small style="color:green;">(In Use)</small>' : ''}<br>
                    <small>For: ${e(i.owner)} | Loc: ${e(i.location)} | Exp: ${e(i.expiry_date)}</small>
                </div>
                <div class="action-group">
                    <button class="btn-sm btn-outline" onclick="updateStatus(${i.id},'in_use')" title="Mark In Use"><svg class="icon"><use href="#icon-play"></use></svg></button>
                    <button class="btn-sm btn-outline" onclick="updateStatus(${i.id},'trash')" title="Delete"><svg class="icon"><use href="#icon-trash"></use></svg></button>
                </div>
            </div>`).join('')}</div>`;
}
async function updateStatus(id, s){ await callApi('update_status', {id:id, status:s}); loadPage('inventory'); }
...
async function renderMyMeds(c) {
    ...
    c.innerHTML = `
        ...
        <div class="card">
            <input type="text" id="myMedName" placeholder="Medication Name">
            <input type="text" id="myMedStr" placeholder="Strength">
            <select id="myMedOwner">${fam.map(f => `<option ${f.is_default == '1' ? 'selected' : ''}>${e(f.name)}</option>`).join('')}</select>
            <button class="btn" style="margin-top:10px;" onclick="addMyMed()">Add Med</button>
        </div>
        <div id="activeMyMeds">${meds.active.map(i => `
...
async function genReport(type) {
    ...
    doc.autoTable({ head: [['Name', 'Strength', 'Owner', 'Expiry']], body: filtered.map(r => [r.name, r.strength, r.owner, r.expiry_date]), startY: 20 });
...
async function renderSystem(c) {
    ...
    // 6. ALL USERS: CHANGE OWN PASSWORD & DATA MANAGEMENT
    html += `
